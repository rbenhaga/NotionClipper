...
export function useTheme(): UseThemeReturn {
  ...
  // ✅ SOLUTION: Fonctions stables avec refs pour éviter les re-renders
    ...
    const loadTheme = async () => {
      try {
        if (window.electronAPI?.getConfig) {
          ...
          let savedTheme = config.theme || 'light';
          if (!config.theme) {
            console.log('🎨 Aucun thème sauvegardé - utilisation du thème clair par défaut');
          }
        ...
        // Fallback sur le mode clair pour le premier lancement
        setThemeState('light');
  ...
  }, [theme]); // ✅ FIX: Seulement dépendre de theme
  ...
  const toggleTheme = useCallback(() => {
    ...
    setTheme(newTheme);
  }, [actualTheme, setTheme]); // ✅ Garder setTheme car c'est stable maintenant
