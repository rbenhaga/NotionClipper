...
class ImageService {
  ...
  async uploadToNotion(imageBuffer: Buffer, filename: string = 'image.png', token: string): Promise<string> {
    if (!token) {
  ...
  async uploadImage(imageBuffer: Buffer, filename: string = 'clipboard-image.png', token: string): Promise<string> {
    return await this.uploadToNotion(imageBuffer, filename, token);
  }

  async processImage(imageBuffer: Buffer, filename: string = 'clipboard-image.png', token: string): Promise<UploadResult> {
    try {
      const fileUploadId = await this.uploadToNotion(imageBuffer, filename, token);
      return {
        success: true,
        fileUploadId,
        type: 'notion_upload'
