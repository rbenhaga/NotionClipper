...
function registerWindowIPC(): void {
  ...
  ipcMain.handle('window-toggle-pin', (event: IpcMainInvokeEvent) => {
    try {
  ...
  ipcMain.handle('window-get-pin-state', (event: IpcMainInvokeEvent) => {
    try {
      const window = BrowserWindow.fromWebContents(event.sender);
      if (!window) {
  ...
  ipcMain.handle('window-set-opacity', (event: IpcMainInvokeEvent, opacity: number) => {
    try {
      const window = BrowserWindow.fromWebContents(event.sender);
      if (!window) {
        return { success: false, error: 'Window not found' };
